...
def detect_needle_tip(tensor, threshold_value=160, roi_height_ratio=0.2, roi_width_ratio=0.2):
...
def visualize(image, predicted_points, confidence=0):
...
def test(device, test_loader):
    ...
    for batch_idx, (left_images, right_images, left_points, right_points, points_3d) in enumerate(test_loader):
        ...
        for i in range(left_images.size(0)):
            ...
            frame = visualize(single_image.cpu().numpy().transpose((1, 2, 0)), batch_outputs[-1])#,confidence[i])
